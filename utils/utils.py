...
def crop_off_image(img, crop_size):
    center_point = (img.shape[0] / 2, img.shape[1] / 2)
    tlX, tlY = int(center_point[0] - crop_size / 2), int(center_point[1] - crop_size / 2)
    return img[tlY: tlY + crop_size, tlX: tlX + crop_size]


def preprocess(*image_path, crop_size=None, max_size=512, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
    ori_imgs = [cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB) for img_path in image_path]

    if crop_size is not None:
        ori_imgs = [crop_off_image(img, crop_size) for img in ori_imgs]

    if mean is None:
        normalized_imgs = [img / 255.0 * 2 - 1 for img in ori_imgs]  # Used pretrained weights
    else:
        normalized_imgs = [(img / 255. - mean) / std for img in ori_imgs]  # Unused pretrained weights
...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold):
    ...
    classification = softmax(classification)
...
def save_checkpoint(model, saved_path):
    if isinstance(model, CustomDataParallel):
        torch.save(model.module.model.state_dict(), saved_path)
    else:
        torch.save(model.model.state_dict(), saved_path)
